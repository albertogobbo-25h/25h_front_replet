# Guia Rápido - Frontend → Backend API

**Objetivo:** Referência rápida das chamadas backend para implementação do frontend.
...
// Usuário autenticado automaticamente após login
```

---

## 1️⃣ ONBOARDING (Após Login/Signup)

### 🔹 Processar Pós-Login

**Quando chamar:** Logo após login/signup bem-sucedido

```typescript
const { data } = await supabase.rpc('processar_pos_login', {
  p_nome: 'João Silva',        // opcional (obrigatório se 1ª vez)
  p_whatsapp: '11987654321'    // opcional (obrigatório se 1ª vez)
})
```
...
### 🔹 Criar Nova Assinatura (Upgrade/Renovação)

**Quando chamar:** Quando usuário escolhe plano pago
...
```typescript
{
  ...
  data?: {
    ...
    cobranca: {
      id: string,           // ⚠️ IMPORTANTE: usar para iniciar pagamento
      valor: number,
      data_vencimento: string
    },
...
if (data.code === 'INCOMPLETE_DATA') {
  // Redirecionar para tela de atualização de cadastro
  // Chamar atualizar_dados_assinante() primeiro
  // Depois tentar criar_nova_assinatura() novamente
}
```

### 🔹 Iniciar Pagamento da Cobrança

**Quando chamar:** Após criar assinatura ou quando usuário clicar em "Pagar"
...
```typescript
{
  ...
  data?: {
    pluggy: {
      id: string,
      paymentUrl: string,        // ⚠️ USAR PARA REDIRECIONAR
      status: string
    },
    database: {
      cobranca_id: string,
...
if (data.status === 'OK') {
  ...
  // Após pagamento, Pluggy retorna e webhook atualiza status automaticamente
...
## 3️⃣ GERENCIAR DADOS DO ASSINANTE
...
**Alternativa temporária:** Contato com suporte

---

## 📊 FLUXO COMPLETO - EXEMPLO PRÁTICO

### **Cenário: Novo usuário faz upgrade de gratuito para pago**
...
if (posLogin.assinatura?.plano.ind_gratuito) {
  ...
  // 4. Usuário escolhe plano e clica em "Assinar"
  ...
  if (novaAssinatura.status === 'OK') {
    ...
    if (pagamento.status === 'OK') {
      ...
      // 9. Após pagamento, webhook atualiza automaticamente
      // Frontend deve fazer polling ou usar realtime para detectar mudança
...
**Opção 2: Supabase Realtime** (Recomendado)

```typescript
// Escutar mudanças na tabela assinatura
...
if (data.code === 'INCOMPLETE_DATA' || data.code === 'DADOS_INCOMPLETOS') {
  ...
  // Mostrar campos obrigatórios: data.required_fields ou data.campos_faltantes
}
```
...
if (data.code === 'PENDING_SUBSCRIPTION_EXISTS') {
  // Já existe assinatura aguardando pagamento
...
if (data.code === 'PAYMENT_IN_PROGRESS') {
  // Já existe requisição de pagamento em andamento
...
if (data.code === 'PLAN_NOT_FOUND' || data.code === 'PLAN_INACTIVE') {
  // Plano não existe ou foi desativado
  // Mostrar lista de planos disponíveis novamente
}
```
...
### Onboarding
- [ ] Chamar `processar_pos_login()` após login
...
- [ ] Completar dados com `atualizar_dados_assinante()` se necessário
...
### Pós-Pagamento
- [ ] Implementar polling ou Realtime para verificar ativação
...
- [ ] Permitir visualização de histórico (incluir_historico: true)

---

## 🚀 PRONTO PARA COMEÇAR!

Este guia cobre todos os endpoints necessários para implementar o fluxo completo de onboarding e gestão de assinaturas.

**Dúvidas?** Consulte a documentação completa em:
- `docs/backend/FLUXO_ONBOARDING.md`
- `docs/backend/FLUXO_GESTÃO_ASSINATURA.md`
